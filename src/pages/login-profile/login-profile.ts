...
export class LoginProfilePage {
  ...
  constructor(
    ...
    this._register = this.formBuilder.group({
      ...
      renewpwd: ['', Validators.compose([Validators.required, Validators.minLength(6), Validators.maxLength(15)])],
  ...
  onClick(v: string) {
    if (v === 'C') {
      ...
      this.ex_user = null;
      this._status = false;
      this._authservice.login_u(this.token, this._zclient, this._user.username, this._user.password, this._user.fullname,this._user.renewpwd)
        .then(
          (d) => {
